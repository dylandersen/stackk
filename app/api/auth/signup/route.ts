...
export async function POST(request: NextRequest) {
  try {
    const { email, firstName, lastName, referralSource } = await request.json();
    ...
    if (!validReferralSources.includes(referralSource)) {
      return NextResponse.json(
    ...
    // Note: We can't easily check if a user exists via InstantDB Admin SDK
    // The check will happen on the client side when they try to sign in
    // For now, we'll generate the code and let the client handle user creation after verification
    ...
    if (emailService === 'console') {
      ...
      console.log('Magic code for signup', email, ':', code);
      console.log('Signup data:', { firstName, lastName, referralSource });
      ...
      try {
        ...
        console.log('Sending signup email via Resend:', { from: fromEmail, to: email });
        ...
        const result = await resendClient.emails.send({
          ...
          html: `
            <div style="font-family: system-ui, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
              <h1 style="color: #333; margin-bottom: 20px;">Welcome to Stackk!</h1>
              ...
              <p style="color: #666; font-size: 14px; margin-bottom: 10px;">
                Enter this code in the signup page to complete your registration.
              </p>
          ...
          `,
          text: `Welcome to Stackk!\n\nUse this code to complete your signup: ${code}\n\nThis code will expire in 15 minutes. If you didn't request this, you can safely ignore this email.`,
        });
    ...
    // Return success with signup data (client will store this temporarily)
    return NextResponse.json({ 
      success: true,
      signupData: {
        firstName,
    ...
    return NextResponse.json(
      { error: error.body?.message || error.message || 'Failed to process signup' },
      { status: 500 }
    );
  }
}
